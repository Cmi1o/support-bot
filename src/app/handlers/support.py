...
async def support_get_msg(message: Message, state: FSMContext) -> None:
    ...
    await message.answer('Если желаете переписать введите "Нет"', reply_markup=kb.confirm)
    
    await state.update_data(message=message) # message data of json: aiogram.types.Message
...
async def support_yes(message: Message, state: FSMContext) -> None:
    ...
    if user and not user.thread_id:
